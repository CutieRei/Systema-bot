...
class Event(commands.Cog):
    ...
    async def on_connect(self):
        if self.ready:
            ...
            async with sql.connect("./db/data.sql"):
    ...
    async def on_ready(self):
        await self.bot.change_presence(
            activity=discord.Activity(type=discord.ActivityType.listening, name="Muted people :)"))
    ...
    async def on_member_join(self, member):
        ...
        if chad is None:
            ...
            sentence = [f"Hello! {member.mention}({str(member)})",
                        f"Grab a bot and have fun in pre-alpha state, {member.mention}({str(member)})"]
            card = Embed(
