...
async def select_user(_id):
    async with sql.connect(db_path) as db:
        async with db.execute("SELECT * FROM data WHERE id = ?", (_id,)) as c:
...
async def update_user(_id, money, level, exp):
    async with sql.connect(db_path) as db:
        await db.execute("UPDATE data SET exp = ?,level = ?,money = ? WHERE id = ?", (exp, level, money, _id,))
        await db.commit()
    return _id


async def user_work(_id):
    user1 = await select_user(_id)
    user = user1.copy()
    ...
    user["level"] = update[1]
    user["money"] = update[2]
    await update_user(_id, user["money"], user["level"], user["exp"])
    return [update, user1]
...
class Game(commands.Cog):
    ...
    async def work_error(self, ctx, err):
        if isinstance(err, commands.CommandOnCooldown):
            ...
            await ctx.send(
                f"Please retry after **{int(minute)}m** **{int(second)}s**\n**{err.cooldown.rate}** uses per \
                **{err.cooldown.type.name.capitalize()}** for every **{int(err.cooldown.per)}s**")
        else:
            raise err
    ...
    async def verify(self, ctx):
        ...
        """
        async with sql.connect(db_path):
    ...
    async def profile(self, ctx, member: discord.Member = None):
        ...
        """
        if member is not None and await select_user(member.id):
            ...
            card.set_footer(icon_url=member.avatar_url_as(static_format='png'),
                            text=f"Copyright© {datetime.utcnow().year} ReyterGTX")
            ...
            card.set_footer(icon_url=ctx.author.avatar_url_as(static_format='png'),
                            text=f"Copyright© {datetime.utcnow().year} ReyterGTX")
            await ctx.send(embed=card)
        else:
